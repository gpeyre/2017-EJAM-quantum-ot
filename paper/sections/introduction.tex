...
Although this is not the topics of this paper, let us note that several approaches have been proposed to define notions of OT between two tensors (without any spatial displacement). Gurvits introduced in~\cite{gurvits2004classical} a Sinkhorn-like algorithm to couple two tensors by an endomorphism that preserves positivity. This algorithm is however only known to converge in the case where the to involved tensors are the identity matrices, see~\cite{georgiou2015positive} for a detailed analysis. 
%
