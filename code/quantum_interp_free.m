function nu = quantum_interp_free(gamma, mu, xy, n, m, options)

% quantum_interp_free - heuristic McCann interpolation-like
%
%   nu = quantum_interp_free(gamma, mu, xy, n, m, options)
%
%   m is number of steps in [0,1] for the interpolation.
%
%   gamma is a matricial coupling
%   mu{1} and mu{2} are the marignals
%   xy{s}(i,:) are the 2D positions of the diracs, assumed to be in [0,1]^2, 
...
for k=1:m
    progressbar(k,m);
    t = (k-1)/(m-1);
    nu{k} = zeros(size(mu{1},1),size(mu{1},2),n*n);
    for i=1:length(I)
        % interpolated value
        G = ( (1-t)*A{1}(:,:,I(i)) + t*A{2}(:,:,J(i)) ) * ...
            gamma_ij(:,:,i);
        % interpolated position
        p = (1-t)*xy{1}(I(i),:) + t*xy{2}(J(i),:);
