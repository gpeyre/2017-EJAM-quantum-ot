...
void mexFunction(int nlhs, mxArray * plhs[], int nrhs, const mxArray *prhs[]) {
	if (nrhs != 1)
        mexErrMsgIdAndTxt("MATLAB:mexcpp:nargin", "TENSORLOG2X2 requires one input.");
    else if (nlhs != 1) // vertices, faces, indicator
        mexErrMsgIdAndTxt("MATLAB:mexcpp:nargout", "TENSORLOG2X2 produces one output.");

    size_t nd = mxGetNumberOfDimensions(prhs[0]);
    if (nd < 2) mexErrMsgTxt("TENSORLOG2X2 requires at least 2 array dimensions.");

    const mwSize *dim = mxGetDimensions(prhs[0]);
    if (dim[0] != 2 || dim[1] != 2)
        mexErrMsgTxt("TENSORLOG2X2 only works for 2x2 matrices.");

    double *data = mxGetPr(prhs[0]);

    size_t total = mxGetNumberOfElements(prhs[0]);
    if (total % 4 != 0)
        mexErrMsgTxt("TENSORLOG2X2 needs a total size divisible by 4.");
